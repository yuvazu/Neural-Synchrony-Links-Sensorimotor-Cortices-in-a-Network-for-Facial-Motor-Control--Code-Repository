%% TwoMonkeys_stats_mod_Chew_vs_LS
% this mfile is a modification of Elie's to assess Coh, PPC, Granger and
% store the Power during the Movement period (500ms) for Chew & LS for the
% rebuttal. 
...
% modify June, 2025 for rebuttal, ok
...
lipsmack_folder = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_LSChew_A/LS_twoM/'; 
chew_folder  = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_LSChew_A/chew_twoM/'; 
...
assert(numel(lipsmack_files)==numel(chew_files)) % make sure equal number of files
...
for j=1:numel(lipsmack_files)
    ...
    s_fname = [s_fname; {lipsmack_files(j).name} {chew_files(j).name}];
    ...
    lf=lipsmack_files(j).name(11:end);
    cf=chew_files(j).name(11:end);

    thisDayC=cf(1:end-17)
    thisDayL=lf(1:end-17)

    % select ROIs (M1m-M3, PMVm-M3, M1-PMVm, M1m-S1, M3-S1, PMVm-S1)
    ...
    cfg.pad = 2;
    freq1_4conn = ft_freqanalysis(cfg, data1ROIs) % LS (both areas)
    freq2_4conn = ft_freqanalysis(cfg, data2ROIs) % Chew (both areas)
    ...
    cfg = [];
    cfg.method = 'granger';
    gran1 = ft_connectivityanalysis(cfg, freq1_4conn) % L both areas
...
legend(['LS ', area1,'-', area2], ['C ', area1,'-', area2]);
...
%legend( 'LS PMV-> M1', 'C PMV-> M1','LS M1--> PMV', 'C M1--> PMV');
lgh4=legend(['LS ', area1, '-->', area2], ['C ', area1, '-->', area2], ['LS ', area2 '-->' area1], ['C ', area2 '-->' area1])
xlabel('Frequency (Hz)', 'Fontsize', 18); ylabel('Granger', 'Fontsize', 18)
title([area1,'-', area2,' Movement  (n=', num2str(length(lipsmack_files)), ' sess 2M, ntr=', num2str(sum(s_trls)), ')'],'FontSize',17); axis tight

% Store Calculations for All sessions for making histograms
StoreSessionDir = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_LSChew_A/PPC_perSession/';
cd(StoreSessionDir)
save([area1,'-', area2, '_LSC_perSess'], 'area1', 'area2','labeld1', 'labeld2','s_trls','s_*')
...
for thisCh=[2:5]
    ...
    if cfg.channel==2 % Coherence
        ...
        n=2;
        storename = ['Coh_FXC_', area1,'-',area2];
        nl=2;
   ...
   % Assess Significant Negative Clusters
    ...
    if cfg.channel<4
        figure(n); gcf; hold on;
        lgh=legend(['LS ' area1,'-', area2], ['C ', area1,'-', area2]);
    ...
    if PrintOn==1 && cfg.channel<4 
        ...
        ax = gca;
        ax.FontSize = 20; 
        cd('/Users/yuriria/Documents/MATLAB/Thor_Analysis/LFP_analysis/Coh/Barney_FromEli/ElieSCode/figs/TwoMonkeys/2M_Mov_ChewLS')
        ...
        % 
        if cfg.channel==2
            save(['Power_FXC_', area1,'-',area2], 's_pow1', 's_pow2','area1', 'area2', ...
