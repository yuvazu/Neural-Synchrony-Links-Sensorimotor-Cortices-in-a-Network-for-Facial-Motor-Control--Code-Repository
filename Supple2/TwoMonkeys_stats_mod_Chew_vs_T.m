%% TwoMonkeys_stats_mod_Chew_vs_T
% this mfile is a modification of Elie's to assess Coh, PPC, Granger and
% store the Power during the Movement period (500ms) for Chew & T for the
% rebuttal. 
% It also corrects the voltage in Thor, to have everything in
% microVolts. 
% Modified to store both Granger directions (sept/2023)
% Modified to store PPC per session for histograms (nov/2024)
% Modified to assess the contrast LS vs Chew 
% Data & Figures stored in:'/Users/yuriria/Documents/MATLAB/Thor_Analysis/tfP_analysis/Coh/Barney_FromEli/ElieSCode/figs/TwoMonkeys/2M_Movement'
...
% modify June, 2025 for rebuttal
% Figures Stored: '/Users/yuriria/Documents/MATLAB/Thor_Analysis/tfP_analysis/Coh/Barney_FromEli/ElieSCode/figs/thor')
...
clear
% define folders for LS vs Chew ( 2 monkeys) 
threat_folder = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_TChew_A/T_twoM/'; % auto
chew_folder  = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_TChew_A/chew_twoM/'; % auto 
...
chew_files = dir(fullfile(chew_folder, '*mat'))
assert(numel(threat_files)==numel(chew_files)) % make sure equal number of files
...
for j=1:numel(threat_files)
    ...
    % load chew file
    load([chew_folder chew_files(j).name])
    disp([chew_files(j).name])
    % rename
    ...
    if isempty(strfind(threat_files(j).name,'T'))==0 % thor files
        [data1]=Get_ThorVoltageInMicroVolts(data1); % *1e6
        [data2]=Get_ThorVoltageInMicroVolts(data2);
        disp(['-----', threat_files(j).name, 'Voltage to microVolts---'])
    elseif isempty(strfind(threat_files(j).name,'T'))==1 % barney files
    end
    ...
    s_fname = [s_fname; {threat_files(j).name} {chew_files(j).name}];
    ...
    cf=chew_files(j).name(11:end);

    thisDayC=cf(1:end-17)
    thisDayL=tf(1:end-17)
    ...
    %cfg.channel = 'M1m*';
    %cfg.channel ='M3*'
    if isempty(strfind(threat_files(j).name,'T'))==0
        cfg.channel = 'PMV*';
    elseif isempty(strfind(threat_files(j).name,'T'))==1 % barney files
        cfg.channel = 'PMVm*';
    end
    ...
    cfg.avgoverchan = 'yes';
    data1_ROI1 = ft_selectdata(cfg, data1); % Threat
    data2_ROI1 = ft_selectdata(cfg, data2); % Chew
    ...
    %cfg.channel='M3*';
    cfg.channel='S1*';
    % if isempty(strfind(chew_files(j).name,'T'))==0
    %      cfg.channel = 'PMV*';
    % elseif isempty(strfind(chew_files(j).name,'T'))==1 % barney files
    %      cfg.channel = 'PMVm*';
    % end

    area2=cfg.channel(1:end-1);
    cfg.avgoverchan = 'yes';
    data1_ROI2 = ft_selectdata(cfg, data1); % Threat
    data2_ROI2 = ft_selectdata(cfg, data2); % chew
    data1_ROI2.label = {'ROI2'};
    data2_ROI2.label = {'ROI2'};

    % combine the 2 ROIs in 1 structure for each mov type
    data1ROIs = ft_appenddata([], data1_ROI1, data1_ROI2); % T both PMV & M1
    data2ROIs = ft_appenddata([], data2_ROI1, data2_ROI2); % C both PMV & M1
    ...
    cfg.pad = 2;
    freq1_4conn = ft_freqanalysis(cfg, data1ROIs) % T (both areas)
    freq2_4conn = ft_freqanalysis(cfg, data2ROIs) % C (both areas)
    ...
    cfg = [];
    cfg.method = 'granger';
    gran1 = ft_connectivityanalysis(cfg, freq1_4conn) % T both areas
    gran2 = ft_connectivityanalysis(cfg, freq2_4conn) % C both areas
   ...
   %%%
    ...
    % store the threat connectivity measures in the existing pow struct
...
legend(['T ', area1,'-', area2], ['C ', area1,'-', area2]);
xlabel('Frequency (Hz)','Fontsize', 18); ylabel('Coherence', 'Fontsize', 18)
title([ area1,'-', area2, '  Movement  (n=',num2str(length(threat_files)), ' sess 2M, ntr=', num2str(sum(s_trls)), ')'],'FontSize',17); axis tight
...
%legend( 'T PMV-> M1', 'C PMV-> M1','T M1--> PMV', 'C M1--> PMV');
lgh4=legend(['T ', area1, '-->', area2], ['C ', area1, '-->', area2], ['T ', area2 '-->' area1], ['C ', area2 '-->' area1])
xlabel('Frequency (Hz)', 'Fontsize', 18); ylabel('Granger', 'Fontsize', 18)
title([area1,'-', area2,' Movement  (n=', num2str(length(threat_files)), ' sess 2M, ntr=', num2str(sum(s_trls)), ')'],'FontSize',17); axis tight

% Store Calculations for All sessions for making histograms
StoreSessionDir = '/Volumes/Seagate/Barney_CollabwElie/TwoMonkeys_TChew_A/PPC_perSession';
cd(StoreSessionDir)
save([area1,'-', area2, '_CT_perSess'], 'area1', 'area2','labeld1', 'labeld2','s_trls','s_*')
...
for thisCh=[2:5]
    ...
    elseif cfg.channel==3 % PPC
        ...
        % modify yvz 24/11/28 for storing per session the PPC --> for
        % histograms
   ...
   % Assess Significant Negative Clusters
    ...
    if cfg.channel<4
        figure(n); gcf; hold on;
        lgh=legend(['T ' area1,'-', area2], ['C ', area1,'-', area2]);
    ...
    if PrintOn==1 && cfg.channel<4 
        ...
        ax = gca;
        ax.FontSize = 20; 
        cd('/Users/yuriria/Documents/MATLAB/Thor_Analysis/LFP_analysis/Coh/Barney_FromEli/ElieSCode/figs/TwoMonkeys/2M_Mov_ChewT')
