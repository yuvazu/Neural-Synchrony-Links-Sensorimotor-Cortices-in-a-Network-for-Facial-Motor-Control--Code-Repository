function S2_DEFJKL_Prototype_SuppleTC_PPC_Grang_TvsC
% this function loads the mean Coh/PPC across all sessions for Threats &
...
% the results were done using TwoMonkeys_stats_mod_Chew_vs_T (14 sessions , 2 monkeys)
% Run and stored % July 17th 2025
clear all
close all

% modify TargetDir
TargetDir='/Users/yuriria/Documents/MATLAB/Thor_Analysis/LFP_analysis/Coh/Barney_FromEli/ElieSCode/figs/TwoMonkeys/2M_Mov_ChewT'
cd(TargetDir)
...
for thisP=1:3
    ...
    plot(stat.freq,var1,'-','linewidth', 2,'Color', [0.8510    0.3255    0.0980]) % red (T)
    ...
    if isfield(stat,'negclusters')==1 && isempty(stat.negclusters)~=1
        for i=1:size(stat.negclusters,2)
            ...
            if alpha<=pvalue % if this cluster is significant
                ...
                plot(stat.freq(stat.negclusterslabelmat==i),var1(idx1), 'Color',[0.8510    0.3255    0.0980,0.6],'LineWidth', lw); % red
    ...
    if isfield(stat,'posclusters')==1 && isempty(stat.posclusters)~=1
        for i=1:size(stat.posclusters,2)
            ...
            if alpha<=pvalue % if this cluster is significant
                ...
                plot(stat.freq(stat.posclusterslabelmat==i),var1(idx1), 'Color',[0.8510    0.3255    0.0980,  0.6],'LineWidth', lw); % red
...
strAA={'M1m-M3', 'M3-M1m','M3-PMVm', 'PMVm-M3','M1m-PMVm', 'PMVm-M1m' }
...
for pair=1:6
    ...
    if pair==2 || pair==3 || pair==5  % for even numbers area1--> area2
        plot(stat.freq,var1,'-','linewidth', 2, 'Color',[0.8510    0.3255    0.0980]); % red
...
for i=1:3
    ...
    if x(i)==4 
        ...
        plot([25 28], [0.47 0.47],'--','LineWidth', 1.5, 'Color',[0.8510    0.3255    0.0980]) % change for red 
        text(30, 0.47,['T-',strAA{thisP(i)-1}]);

        plot([25 28], [0.45 0.45],'--', 'LineWidth', 1.5, 'Color',[0         0.4471    0.7412]) % change for blue
        text(30, 0.45,['C-',strAA{thisP(i)-1}]);

        plot([25 28], [0.40 0.40],'-','LineWidth', 1.5, 'Color',[0.8510    0.3255    0.0980]) % change for red
        text(30, 0.40,['T-',strAA{thisP(i)}])

        plot([25 28], [0.37 0.37],'-','LineWidth', 1.5, 'Color',[0         0.4471    0.7412]) % change for blue
        text(30, 0.37,['C-',strAA{thisP(i)}])
        ylim([0 0.6])

    elseif x(i)==5 || x(i)==6
       
        plot([25 28], [0.45 0.45],'-','LineWidth', 1.5, 'Color',[0.8510    0.3255    0.0980]) % yellow 
        text(30, 0.45,['T-',strAA{thisP(i)-1}]);

        plot([25 28], [0.40 0.40],'-', 'LineWidth', 1.5, 'Color',[0         0.4471    0.7412]) % smoked red
        text(30, 0.40,['C-',strAA{thisP(i)-1}]);

        plot([25 28], [0.35 0.35],'--','LineWidth', 1.5, 'Color',[0.8510    0.3255    0.0980]) % yellow 
        text(30, 0.35,['T-',strAA{thisP(i)}])
...
% 
if PrintOn==1
    print(gcf,'-depsc', 'SuppleCT_PPC-Granger_r1')
    print('SuppleCT_PPC-Granger_r1','-dsvg')
    savefig('SuppleCT_PPC-Granger_r1.fig')
    close
end
